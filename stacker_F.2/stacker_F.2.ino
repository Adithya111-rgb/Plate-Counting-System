...
void OnDataRecv(const esp_now_recv_info *info, const uint8_t *data, int len) {
  ...
  // Standard clear command from Master logic
  if (incoming.code == CMD_STACK_CLEAR) {
    state = S_FULL_STACK_CLEAR;
    readySent = false;
  }

  // Master Reset Handshake Logic
  if (incoming.code == MSG_GET_STATUS) {
    if (digitalRead(SENSOR_TOP) == LOW) {
      // Physically at top sensor: just report ready
      sendReady();
      readySent = true;
    } else {
      // Not at top: Trigger auto-clear to evacuate plates
      state = S_FULL_STACK_CLEAR;
      readySent = false;
    }
  }
...
void loop() {
  ...
  // Edge detection for Automatic Plate movement
  lastPlate = plate;
  ...
  if (millis() - lastLCD > 300) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("SH:"); lcd.print(sharpVal);
    lcd.print(" M:"); lcd.print(moving ? "Y" : "N");
    lcd.print("    ");
    lastLCD = millis();
  }
}
